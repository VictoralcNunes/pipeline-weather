...
# igual a '../data/weather_data.json'
...
def normalize_weather_colummns(df: pd.DataFrame) -> pd.DataFrame:
    df_weather = pd.json_normalize(df['weather'].apply(lambda x: x[0])) # pyright: ignore[reportArgumentType]
    ...
    logging.info(f"\n Colunas 'weather' normalizadas - {len(df.columns)} colunas no total")
...
def rename_columns(df: pd.DataFrame, columns_names: dict[str,str]) -> pd.DataFrame:
    logging.info(f"\n -> Renomeando colunas: {columns_names}")
...
def data_transformations() -> pd.DataFrame:
    print("\n Iniciando transformações de dados...")
    df = create_dataframe(path_name)
    df = normalize_weather_colummns(df)
